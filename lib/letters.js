...
import { z } from "zod";
import { addUserToAddressbook, getUserByEmail } from '@/lib/users';
...
export async function send(letterData) {
    try {
        ...
        if (parsedData.success) {  
            //note parsedData only contains the result, not the original data, use letterData to access the original data         
            const data = await saveData(letterData);

            if (data.success) {
                await addUserToAddressbook(letterData.senderId, letterData.recipientEmail);
            }
