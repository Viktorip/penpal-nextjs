...
const AddressbookList = ({ list, clickHandler }) => {
    ...
    return (
        <div className="absolute sm:top-full max-sm:-top-full left-1 z-20 border-2 border-black rounded-e-lg rounded-es-lg overflow-hidden">
            {list.map(item => (<div className="flex flex-row items-center bg-white border-b-2 last:border-b-0 border-blue-500 cursor-pointer hover:bg-gray-200" key={item.email} onClick={() => clickHandler(item.email)}>
                <div>
                    <RiFileUserFill className="text-2xl text-blue-500" />
                </div>
                <div className="flex flex-col justify-center pl-1 w-auto min-w-[20rem] h-10 text-md text-indigo-900 leading-none" ><div>{item.fullname} ({item.email})</div></div>
            </div>))}
        </div>
    )
}